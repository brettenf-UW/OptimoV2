...
def lambda_handler(event, context):
    """
    Handler for the Lambda function that processes optimization results and returns metrics.
    ...
    try:
        ...
        # Check if metrics are already calculated and stored in DynamoDB
        if 'metrics' in job_item:
            logger.info("Using cached metrics from DynamoDB")
            metrics = job_item['metrics']
            download_urls = generate_download_urls(job_item.get('results', []))
        ...
        metrics = calculate_metrics(job_id, job_item.get('results', []))
        
        # Store metrics in DynamoDB for future use
        table.update_item(
            Key={'jobId': job_id},
...
def generate_download_urls(result_files):
    """
    Generate presigned URLs for downloading result files
    """
    download_urls = {}
...
def calculate_metrics(job_id, result_files):
    """
    Calculate metrics from CSV files in S3
    """
    ...
    try:
        ...
        master_schedule_key = next((f for f in result_files if f.endswith('Master_Schedule.csv')), None)
        student_assignments_key = next((f for f in result_files if f.endswith('Student_Assignments.csv')), None)
        violations_key = next((f for f in result_files if f.endswith('Constraint_Violations.csv')), None)
        ...
        master_schedule_df = download_csv(master_schedule_key)
        student_assignments_df = download_csv(student_assignments_key)
        violations_df = download_csv(violations_key) if violations_key else pd.DataFrame()
        ...
        logger.info(f"Successfully calculated metrics: {json.dumps(metrics)}")
        return metrics
