...
# Get environment variables first, before initializing AWS clients
...
def run_optimization(file_paths):
    ...
    try:
        ...
        if not gemini_api_key:
            ...
            try:
                response = secretsmanager.get_secret_value(SecretId='optimo/gemini-api-key')
                gemini_api_key = response['SecretString']
            except:
                logger.warning("No Gemini API key found, using empty string")
                gemini_api_key = ""
        
        # Run using the command line interface
        cmd = [
            'python', '/app/scripts/run_pipeline.py',
            '--student-info', file_paths['student_info'],
            '--student-prefs', file_paths['student_preferences'],
            '--teacher-info', file_paths['teacher_info'],
            '--teacher-unavail', file_paths['teacher_unavailability'],
            '--sections-info', file_paths['sections_info'],
            '--max-iterations', str(MAX_ITERATIONS),
            '--output-dir', '/tmp/output',
            '--api-key', gemini_api_key
        ]
        
        # Add period file if provided
        if 'period' in file_paths and file_paths['period']:
            cmd.extend(['--period-info', file_paths['period']])
        
        # Run the pipeline
        logger.info(f"Running pipeline with command: {' '.join(cmd[:10])}...")
        ...
        for file in output_dir.glob('*.csv'):
            result_files[file.name] = str(file)
